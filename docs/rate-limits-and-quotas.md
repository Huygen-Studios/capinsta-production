...
Duration policy and upload bytes are separate controls. Caption-duration failures use
`caption_duration_limit_exceeded`; multipart byte failures use `upload_too_large`.
Set the reverse proxy request-body limit to at least `MAX_UPLOAD_SIZE_MB` plus 2 MB
of multipart overhead so the backend can return the structured application error.
