...
import {
	CapinstaCaptionNode,
	type ResolvedCapinstaCaptionNodeState,
...
async function resolveNode({
	...
	if (node instanceof VideoNode) {
		...
		node.resolved = resolveCapinstaCaptionNode({ node, context });
...
function resolveCapinstaCaptionNode({
	node,
	context,
}: {
	node: CapinstaCaptionNode;
	context: ResolveContext;
}): ResolvedCapinstaCaptionNodeState | null {
	const timeSeconds = mediaTimeToSeconds({
		time: roundMediaTime({ time: context.time }),
	});
	...
	if (!renderData || !renderData.renderText.trim()) {
		return null;
	}

	const transform = buildTransformFromExportParams(renderData);
	...
	return {
		...
		opacity: 1,
		textColor:
			typeof renderData.style.textParams.color === "string"
				? renderData.style.textParams.color
				: "#ffffff",
		backgroundColor:
			typeof renderData.style.textParams["background.color"] === "string"
				? renderData.style.textParams["background.color"]
				: "transparent",
		effectPasses: [],
		activeWordIds: getActiveCapinstaExportWordIdsAtTime({
			renderData,
			timeSeconds,
		}),
		timeSeconds,
...
function resolveTextNode({
	...
	const activeCapinstaWordIds = getActiveCapinstaExportWordIdsAtTime({
		renderData: node.params.capinstaExport ?? null,
