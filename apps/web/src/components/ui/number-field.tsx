...
function ScrubbableNumberField({
	...
	return (
		<div
			className={cn(
				"scrubbable-number-field flex h-8 w-full min-w-0 items-stretch overflow-hidden rounded-sm border border-border bg-input text-sm transition-colors",
		...
		>
			{icon && (
				<button
					...
					className="scrubbable-number-field__handle grid min-w-8 shrink-0 cursor-ew-resize select-none place-items-center border-r border-border px-2 text-xs font-semibold text-muted-foreground hover:bg-primary/20 hover:text-foreground"
			...
			<div className="grid min-w-0 flex-1 grid-cols-[minmax(3.5rem,1fr)_minmax(2.35rem,auto)] items-stretch">
				...
				{unitText && (
					<span
						aria-hidden="true"
						className={cn(
							"scrubbable-number-field__unit grid min-w-9 shrink-0 select-none place-items-center border-l border-border bg-muted px-2 text-xs font-semibold tabular-nums text-muted-foreground",
							suffixClassName,
						)}
					>
						{unitText}
