...
export function SignUpForm({ redirectPath }: { redirectPath: string }) {
	...
	const submit = async (event: React.FormEvent) => {
		...
		if (signUpError) {
			setError(readableAuthError(signUpError, "Unable to create your account."));
			setLoading(false);
			return;
		}
		if (!data.session) {
	...
	if (checkEmail) {
		return (
			<AuthShell title="Check your email" description="We sent you a verification link. Open it to finish creating your Capinsta account.">
