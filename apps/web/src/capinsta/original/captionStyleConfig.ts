...
} from "../fonts/captionFontRegistry";

export const CREATOR_FONTS = CAPINSTA_CREATOR_FONTS;
export const FONT_STACKS = CAPINSTA_FONT_STACKS;
...
export const MODERN_MINIMALIST_BASE_CONFIG: CaptionStyleConfig = {
	...
	bigFontFamily: "Montserrat",
	smallFontFamily: "Montserrat",
	fontSize: 112,
	fontWeight: 900,
	textColor: "#FFFFFF",
...
function safeFont(value: unknown) {
	return typeof value === "string" && value in FONT_STACKS
		? value
		: DEFAULT_WORD_HIGHLIGHT_BOX_CONFIG.fontFamily;
}
...
function safeMaxLines(value: unknown) {
	return value === "auto" || value === 1 || value === 2 || value === 3
		? value
		: DEFAULT_WORD_HIGHLIGHT_BOX_CONFIG.maxLines;
}
...
export function resolveFontFamily(fontFamily: string) {
	return (
		...
		FONT_STACKS[DEFAULT_WORD_HIGHLIGHT_BOX_CONFIG.fontFamily]
	);
}
...
export function backgroundRgba(config: CaptionStyleConfig) {
	const hex = safeColor(
		...
		DEFAULT_WORD_HIGHLIGHT_BOX_CONFIG.backgroundColor,
	).replace("#", "");
	const fullHex =
		hex.length === 3
			? hex
...
export function normalizeCaptionStyleConfig(
	...
	const inferredWordEffect =
		raw?.wordEffect ||
		(merged.activeWordBackgroundEnabled ? "highlight" : defaults.wordEffect);
	const wordEffect = safeWordEffect(inferredWordEffect);
	const activeWordColor = safeColor(
		merged.activeWordColor,
		defaults.activeWordColor,
	);
	const activeWordBackgroundColor = safeColor(
		wordEffect === "highlight" && !raw?.activeWordBackgroundColor
			? activeWordColor
			: merged.activeWordBackgroundColor,
		defaults.activeWordBackgroundColor,
	);
	const bigFontSizePx = clamp(
		merged.bigFontSizePx,
		80,
		400,
		defaults.bigFontSizePx || BUILD_BIG_FONT_SIZE_PX,
	);
	const smallFontSizePx = Math.min(
		bigFontSizePx,
		clamp(
			merged.smallFontSizePx,
			20,
			160,
			defaults.smallFontSizePx || BUILD_SMALL_FONT_SIZE_PX,
		),
	);

	return {
		presetName:
			typeof merged.presetName === "string" && merged.presetName.trim()
				? merged.presetName
				: defaults.presetName,
		...
		fontWeight:
			Math.round(
				clamp(
					...
					100,
					1000,
					Number(defaults.fontWeight) || 900,
				) / 50,
			) * 50,
		...
		textStrokeColor: safeColor(
			merged.textStrokeColor,
			defaults.textStrokeColor,
		),
		textStrokeWidth: clamp(
			merged.textStrokeWidth,
			0,
			8,
			defaults.textStrokeWidth,
		),
		textShadowColor: safeColor(
			merged.textShadowColor,
			defaults.textShadowColor,
		),
		textShadowOpacity: clamp(
			merged.textShadowOpacity,
			0,
			1,
			defaults.textShadowOpacity,
		),
		textShadowBlur: clamp(
			merged.textShadowBlur,
			0,
			24,
		...
		activeWordBackgroundOpacity: clamp(
			merged.activeWordBackgroundOpacity,
			0,
			1,
			defaults.activeWordBackgroundOpacity,
		),
		activeWordBackgroundPaddingX: clamp(
			merged.activeWordBackgroundPaddingX,
			0,
			28,
			defaults.activeWordBackgroundPaddingX,
		),
		activeWordBackgroundPaddingY: clamp(
			merged.activeWordBackgroundPaddingY,
			0,
			18,
			defaults.activeWordBackgroundPaddingY,
		),
		activeWordBackgroundBorderRadius: clamp(
			merged.activeWordBackgroundBorderRadius,
			0,
			28,
			defaults.activeWordBackgroundBorderRadius,
		),
		wordEffect,
		animationType:
			merged.animationType === "bounce" || merged.animationType === "none"
				? merged.animationType
				: "pop",
		animationStrength: clamp(
			merged.animationStrength,
			0,
			1.4,
			defaults.animationStrength,
		),
		...
		animationSmoothness: clamp(
			merged.animationSmoothness,
			0,
			1,
			defaults.animationSmoothness,
		),
		...
		backgroundBorderColor: safeColor(
			merged.backgroundBorderColor,
			defaults.backgroundBorderColor,
		),
		backgroundBorderWidth: clamp(
			merged.backgroundBorderWidth,
			0,
			8,
			defaults.backgroundBorderWidth,
		),
		backgroundShadowColor: safeColor(
			merged.backgroundShadowColor,
			defaults.backgroundShadowColor,
		),
		backgroundShadowOpacity: clamp(
			merged.backgroundShadowOpacity,
			0,
			1,
			defaults.backgroundShadowOpacity,
		),
		backgroundShadowBlur: clamp(
			merged.backgroundShadowBlur,
			0,
			60,
			defaults.backgroundShadowBlur,
		),
		backgroundShadowDistance: clamp(
			merged.backgroundShadowDistance,
			0,
			36,
			defaults.backgroundShadowDistance,
		),
		backgroundShadowAngle: clamp(
			merged.backgroundShadowAngle,
			0,
			360,
			defaults.backgroundShadowAngle,
		),
		...
		asymmetricScaleStrength: clamp(
			merged.asymmetricScaleStrength,
			0,
			1,
			defaults.asymmetricScaleStrength || 0,
		),
		randomTiltEnabled: Boolean(merged.randomTiltEnabled),
		smartHighlightEnabled: Boolean(merged.smartHighlightEnabled),
		emphasisGreenColor: safeColor(
			merged.emphasisGreenColor,
			defaults.emphasisGreenColor || "#00FF00",
		),
		emphasisYellowColor: safeColor(
			merged.emphasisYellowColor,
			defaults.emphasisYellowColor || "#FFFF00",
		),
		emphasisRedColor: safeColor(
			merged.emphasisRedColor,
			defaults.emphasisRedColor || "#FF0000",
		),
		revealDuration: clamp(
			merged.revealDuration,
			0.08,
			0.9,
			defaults.revealDuration || 0.28,
		),
		revealYOffset: clamp(
			merged.revealYOffset,
			0,
			80,
			defaults.revealYOffset || 30,
		),
		revealBlur: clamp(merged.revealBlur, 0, 40, defaults.revealBlur || 25),
		phraseHoldDuration: clamp(
			merged.phraseHoldDuration,
			0,
			2,
			defaults.phraseHoldDuration || 0.2,
		),
		bigFontSizePx,
		smallFontSizePx,
		anchorSizeMultiplier: clamp(
			merged.anchorSizeMultiplier,
			0.8,
			2,
			defaults.anchorSizeMultiplier || 1.55,
		),
		supportSizeMultiplier: clamp(
			merged.supportSizeMultiplier,
			0.18,
			0.6,
			defaults.supportSizeMultiplier || 0.28,
		),
		layoutMode: safeLayoutMode(merged.layoutMode),
		layoutAsymmetry: clamp(
			merged.layoutAsymmetry,
			0,
			1,
			defaults.layoutAsymmetry || 0.45,
		),
		layoutSafeMarginPercent: clamp(
			merged.layoutSafeMarginPercent,
			0,
			20,
			defaults.layoutSafeMarginPercent || 8,
		),
		collisionPadding: clamp(
			merged.collisionPadding,
			0,
			120,
			defaults.collisionPadding || 8,
		),
...
export function normalizeModernMinimalistStyleConfig(
	...
	const presetName =
		typeof raw?.presetName === "string" ? raw.presetName.toLowerCase() : "";
	const isBuildConfig =
	...
	return {
		...normalized,
		backgroundShadow:
			normalized.backgroundEnabled && normalized.backgroundShadow,
		backgroundBorderEnabled:
			normalized.backgroundEnabled && normalized.backgroundBorderEnabled,
	};
}
