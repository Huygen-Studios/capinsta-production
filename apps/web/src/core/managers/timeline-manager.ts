...
import { syncCapinstaCaptionDocumentsFromTimeline } from "@/capinsta/captionTimelineSync";
...
export class TimelineManager {
	...
	splitElements({
		...
		const elementsWithLinkedMedia = activeScene
		...
		const command = new SplitElementsCommand({
			elements: elementsWithLinkedMedia,
			splitTime,
			retainSide,
		});
		this.editor.command.execute({ command });
	...
	updateTracks(newTracks: SceneTracks): void {
		...
		const records = project?.capinstaCaptionDocuments ?? [];
		if (records.length > 0) {
			const nextRecords = syncCapinstaCaptionDocumentsFromTimeline({
				records,
				afterTracks: newTracks,
				editedAt: new Date().toISOString(),
			});
			if (nextRecords !== records) {
				this.editor.project.replaceCapinstaCaptionDocuments({
					records: nextRecords,
				});
			}
		}
